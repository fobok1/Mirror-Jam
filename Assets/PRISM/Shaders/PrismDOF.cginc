...
float4 sampletexLin( sampler2D dofSamplr, float2 uv )
{
	//srgb2lin()
    return  tex2D( dofSamplr, uv);    
}

float3 GetBlurColorWithBokeh(sampler2D dofSamplr, float2 blurVec, float2 fragCoord, float ifBackZero)
{
    float2 uv = fragCoord / _MainTex_TexelSize.zw; 
    
    float4 origCol = tex2D(dofSamplr, uv);
    ...
    {
        float4 sample = sampletexLin(dofSamplr, p);// - THRESHOLD ) / (1.0-THRESHOLD);
...
float3 GetBlurColorNoBokeh(sampler2D dofSamplr, float2 blurVec, float2 fragCoord, float ifBackZero)
{
    float2 uv = fragCoord / _MainTex_TexelSize.zw;
    ...
    for (int i=0;i<NUM_SAMPLES;++i)
    {
    	float4 sample = sampletexLin(dofSamplr, p);
...
{		
    ...
    return float4( GetBlurColorWithBokeh(_MainTex, blurvec, fragCoord, 0.0), tex2D(_MainTex, uv).a );
}
...
{		
    ...
    return float4( GetBlurColorNoBokeh(_MainTex, blurvec, fragCoord, 0.0), tex2D(_MainTex, uv).a );
}
//
...
{		
    ...
    float3 sumcol = GetBlurColorNoBokeh(_MainTex, blurvec, fragCoord, 0.0);
    
    float3 dof1 = srgb2lin(tex2D(_DofBlur1, uv).rgb);
    float3 dof2 = srgb2lin(tex2D(_DofBlur2, uv).rgb);
    
    //FOUND THE ***
    col.rgb = sumcol + dof1 + dof2;
    col.rgb = col.rgb / (3.);
    
    return float4(lin2srgb(col.rgb), tex2D(_MainTex, uv).a);
}
...
{		
    ...
    col.rgb = GetBlurColorWithBokeh(_DoF1, blurvecOne, fragCoord, 0.0);
    col2.rgb = GetBlurColorWithBokeh(_DoF2, blurvecTwo, fragCoord, 1.0);
    
    col.a = tex2D(_DoF1, uv).a;
    col2.a = tex2D(_DoF2, uv).a;
